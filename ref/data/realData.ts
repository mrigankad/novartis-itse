...
import rawData from "./incident-data.json";
import kpiData from "./incident_kpis.json";
...
// Transform raw data
export const allTickets: Ticket[] = (rawData as any[]).map((r) => {
...
export function calculateKPIs(tickets: Ticket[]) {
    // If no filters are applied, we could potentially return the static KPI data,
    // but to support filtering, we should calculate dynamic KPIs from the tickets list.
    // HOWEVER, the user provided 'incident_kpis.json', so strictly we should use that
    // for the global view.
    // For now, let's implement dynamic calculation to be responsive to filters.
    ...
    // Reopen rate - difficult to calculate from flat list without history, 
    // but we can look at "Reopen count" from raw data if we had it in Ticket.
    // I didn't map Reopen count to Ticket. Let's assume calculated or use KPI data if all tickets.
    // For dynamic, let's return 0 or map it properly.
    // The raw data HAS "Reopen count". I should add it to Ticket if I want accuracy.
    ...
    return {
        totalTickets,
        backlogTickets,
        reopenRate: kpiData.reopenRate ? (kpiData.reopenRate * 100).toFixed(2) : "0.0", // Fallback to static
        mttr,
        highHopTickets: kpiData.highHopTickets, // Fallback to static
        slaMetRate
    };
}
...
export function getTicketInflowTrend(tickets: Ticket[], dateRange: string = "30d") {
    ...
    return Object.entries(groups).map(([key, data]) => {
        let dateLabel = "";
        if (useHourly) {
             dateLabel = data.dateObj.toLocaleTimeString("en-US", { hour: '2-digit', minute: '2-digit', hour12: false });
        } else {
             dateLabel = data.dateObj.toLocaleDateString("en-US", { year: 'numeric', month: 'short', day: 'numeric' });
        }
        return {
            date: dateLabel,
...
export function getMTTRTrend(tickets: Ticket[], dateRange: string = "30d") {
    ...
    tickets.filter(t => t.resolvedAt).forEach(t => {
        ...
        if (!isNaN(d.getTime())) {
            ...
            if (!groups[key]) {
                groups[key] = { total: 0, count: 0, dateObj: d };
            }
